...
{
    shmemi_signal_set((__gm__ int32_t *)shmem_ptr(addr, pe), val);
}
...
SHMEM_DEVICE void shmemi_highlevel_signal_set(__gm__ int32_t *dst, __gm__ int32_t *src, int pe)
{
    ...
    shmemi_roce_write((__gm__ uint8_t*)shmem_roce_ptr(dst, pe), (__gm__ uint8_t*)src, pe, 0, sizeof(int32_t),
...
SHMEM_DEVICE void shmemi_signal_add(__gm__ int32_t *addr, int pe, int32_t val)
{
    ...
    st_atomic<int32_t>(val, (__gm__ int32_t *)shmem_ptr(addr, pe));
    dcci_atomic();
}
