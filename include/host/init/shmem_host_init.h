...
/**
 * @brief Configure the process-wide number of SDMA QPs created per peer.
 *
 * @note SDMA QP counts are limited to the device's vector-core count (and ACLSHMEM_MAX_AIV_PER_NPU);
 *       SDMA defaults to one QP. Call this interface before initialization. The value must be
 *       identical on every PE; inconsistent values produce incompatible metadata layouts. This
 *       function is thread-safe within a process.
 *
 * @param engine              [in] Data operation engine; only SDMA is supported on this branch.
 * @param qp_num              [in] Number of SDMA QPs, in [1, ACLSHMEM_MAX_AIV_PER_NPU].
