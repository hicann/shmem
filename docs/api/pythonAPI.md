...
8. **put_signal** — 从本地 PE 复制连续数据到指定 PE 的对称内存地址，并在完成后更新远程信号变量。当前仅支持 MTE（Memory Transfer Engine）。同步（blocking）接口。
    ...
    |signal_operation|[in]|信号变量更新操作。支持：``SIGNAL_SET`` / ``SIGNAL_ADD``|
    |remote_pe|[in]|远程 PE 编号，默认 -1。``-1`` 仅为占位默认值，无特殊语义，实际调用时必须传入有效的 PE 编号（0 ~ n_pes-1），否则行为未定义|
    |stream|[in]|预留参数，忽略。底层使用默认流|
    |返回值|-|无返回值|

9. **put** — 在指定流上将本地 PE 的连续数据复制到远程 PE 的对称内存地址。调用者需同步流以确保传输完成。当前仅支持 MTE（Memory Transfer Engine）。非阻塞（non-blocking）接口。
...
10. **get** — 在指定流上将远程 PE 对称内存中的连续数据复制到本地缓冲区。调用者需同步流以确保传输完成。当前仅支持 MTE（Memory Transfer Engine）。非阻塞（non-blocking）接口。
...
11. **signal_op** — 在指定的 PE 上对远程信号变量执行原子操作，操作在给定流上执行。调用者需同步流以观察结果。当前仅支持 MTE（Memory Transfer Engine）。非阻塞（non-blocking）接口。
    ...
    |signal_operation|[in]|对远程信号执行的操作。支持：``SIGNAL_SET`` / ``SIGNAL_ADD``|
    |remote_pe|[in]|待更新远程信号变量所在的 PE 编号，默认 -1。``-1`` 仅为占位默认值，无特殊语义，实际调用时必须传入有效的 PE 编号（0 ~ n_pes-1），否则行为未定义|
    |stream|[in]|ACL 流对象，用于执行排序。传入 ``None`` 将引发 ``AclshmemInvalid`` 异常|
    |返回值|-|无返回值。若 ``stream`` 为 ``None`` 则引发 ``AclshmemInvalid``|

12. **signal_wait** — 等待对称信号变量满足指定比较条件。等待操作在给定流上执行，调用在 host 侧立即返回。同步流后，条件 ``signal_var`` ``cmp`` ``signal_val`` 保证为真。当前仅支持 MTE（Memory Transfer Engine）。
    ...
    |signal_operation|[in]|比较操作符。支持：``CMP_EQ`` / ``CMP_NE`` / ``CMP_GT`` / ``CMP_GE`` / ``CMP_LT`` / ``CMP_LE``|
    |stream|[in]|ACL 流对象，用于执行排序。传入 ``None`` 将引发 ``AclshmemInvalid`` 异常|
    |返回值|-|无返回值。若 ``stream`` 为 ``None`` 则引发 ``AclshmemInvalid``|

13. **quiet** — 确保所有先前发出的对称数据操作在给定流上完成。quiet 操作排入指定流中，调用在 host 侧立即返回；调用者需同步流以观测完成。当前仅支持 MTE（Memory Transfer Engine）。
...
62. **aclshmemx_signal_op_on_stream** — 在指定流上对远程信号变量执行原子操作。当前仅支持 MTE。非阻塞（non-blocking）接口。
...
63. **aclshmemx_signal_wait_until_on_stream** — 非阻塞（non-blocking）接口。在指定流上等待信号变量满足比较条件。调用在 host 侧立即返回，等待在流上执行。调用者需同步流以确保等待完成。当前仅支持 MTE。
...
64. **aclshmemx_quiet_on_stream** — 确保所有先前发出的对称数据操作在给定流上完成。quiet 操作排入指定流中，调用在 host 侧立即返回；调用者需同步流以观测完成。当前仅支持 MTE。
...
以下接口由 ``shmem`` 包顶层（``shmem/__init__.py``）提供，可直接通过 ``shmem.<接口名>`` 调用。与前述 ``shmem.core``/``shmem._pyshmem`` 章节中由 pybind11 直接导出的绑定接口不同，本章节接口是在 Python 层实现的封装：底层组合调用前述绑定接口（如 ``aclshmem_malloc``/``aclshmem_free``），并叠加 torch.Tensor 的构造与元信息维护逻辑。
