    ...
    public:                                                                                                          \
        ...
        {                                                                                                            \
            ...
            aclshmemx_mte_put_nbi(                                                                                   \
                dst_gm[dst_offset * 0], buf_tensor[src_offset * 0], copy_params, (rank + 1) % rank_size, EVENT_ID0); \
            aclshmemx_mte_put_nbi(                                                                                   \
                gva_gm + dst_offset * 1, buf + src_offset * 1, copy_params, (rank + 1) % rank_size, EVENT_ID0);      \
            aclshmem_##NAME##_put_nbi(                                                                               \
                dst_gm[dst_offset * 2], buf_tensor[src_offset * 2], copy_params, (rank + 1) % rank_size);            \
            aclshmem_##NAME##_put_nbi(                                                                               \
                gva_gm + dst_offset * 3, buf + src_offset * 3, copy_params, (rank + 1) % rank_size);                 \
                                                                                                                     \
...
ACLSHMEM_FUNC_TYPE_KERNEL(UB_PUT_NUM_NON_CONTIGUOUS_TEST);

#define TEST_UB_NON_CONTIGUOUS_PUT(NAME, TYPE)                                                                     \
    void test_ub_##NAME##_non_contiguous_put(                                                                      \
    ...
    public:                                                                                                          \
        ...
        {                                                                                                            \
            ...
            aclshmemx_mte_get_nbi(                                                                                   \
                buf + dst_offset * 0, gva_gm + src_offset * 0, copy_params, (rank + 1) % rank_size, EVENT_ID0);      \
            aclshmemx_mte_get_nbi(                                                                                   \
                buf_tensor[dst_offset * 1], src_gm[src_offset * 1], copy_params, (rank + 1) % rank_size, EVENT_ID0); \
            aclshmem_##NAME##_get_nbi(                                                                               \
                buf + dst_offset * 2, gva_gm + src_offset * 2, copy_params, (rank + 1) % rank_size);                 \
            aclshmem_##NAME##_get_nbi(                                                                               \
                buf_tensor[dst_offset * 3], src_gm[src_offset * 3], copy_params, (rank + 1) % rank_size);            \
                                                                                                                     \
            AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE3>(EVENT_ID0);                                              \
            AscendC::WaitFlag<AscendC::HardEvent::MTE2_MTE3>(EVENT_ID0);                                             \
                                                                                                                     \
...
ACLSHMEM_FUNC_TYPE_KERNEL(UB_GET_NON_CONTIGUUS_NUM_TEST);

#define TEST_UB_NON_CONTIGUOUS_GET(NAME, TYPE)                                                                     \
    void test_ub_##NAME##_non_contiguous_get(                                                                      \
        uint32_t block_dim, void* stream, uint64_t config, uint8_t* gva, uint8_t* dev, int repeat, int length)     \
    {                                                                                                              \
        ub_##NAME##_get_non_contiguous_num_test<<<block_dim, nullptr, stream>>>(gva, dev, repeat, length, config); \
    }

ACLSHMEM_FUNC_TYPE_KERNEL(TEST_UB_NON_CONTIGUOUS_GET);
