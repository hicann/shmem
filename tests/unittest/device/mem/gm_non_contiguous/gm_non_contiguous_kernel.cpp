    ...
    public:                                                                                                      \
        ...
        {                                                                                                        \
            ...
            aclshmemx_mte_put_nbi(                                                                               \
                gva_gm + dst_offset * 0, dev_gm + src_offset * 0, buf, (uint32_t)ub_size, copy_params,           \
                (rank + 1) % rank_size, EVENT_ID0);                                                              \
            AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                          \
            AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                         \
            aclshmemx_mte_put_nbi(                                                                               \
                dst_gm[dst_offset * 1], src_gm[src_offset * 1], buf_tensor, copy_params, (rank + 1) % rank_size, \
                EVENT_ID0);                                                                                      \
            AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                          \
            AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                         \
            aclshmem_##NAME##_put_nbi(                                                                           \
                gva_gm + dst_offset * 2, dev_gm + src_offset * 2, copy_params, (rank + 1) % rank_size);          \
            AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                          \
            AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                         \
            aclshmem_##NAME##_put_nbi(                                                                           \
                dst_gm[dst_offset * 3], src_gm[src_offset * 3], copy_params, (rank + 1) % rank_size);            \
                                                                                                                 \
...
ACLSHMEM_FUNC_TYPE_KERNEL(KERNEL_PUT_NUM_NON_CONTIGUOUS);

#define PUT_NON_CONTIGUOUS_NUM_TEST(NAME, TYPE)                                 \
    extern "C" __global__ __aicore__ void put_##NAME##_non_contiguous_num_test( \
...
ACLSHMEM_FUNC_TYPE_KERNEL(PUT_NON_CONTIGUOUS_NUM_TEST);

#define TEST_NON_CONTIGUOUS_PUT(NAME, TYPE)                                                                     \
    void test_##NAME##_non_contiguous_put(                                                                      \
        uint32_t block_dim, void* stream, uint64_t config, uint8_t* gva, uint8_t* dev, int repeat, int length)  \
    {                                                                                                           \
        put_##NAME##_non_contiguous_num_test<<<block_dim, nullptr, stream>>>(gva, dev, repeat, length, config); \
    }

ACLSHMEM_FUNC_TYPE_KERNEL(TEST_NON_CONTIGUOUS_PUT);
    ...
    public:                                                                                                      \
        ...
        {                                                                                                        \
            ...
            aclshmemx_mte_get_nbi(                                                                               \
                dev_gm + dst_offset * 0, gva_gm + src_offset * 0, buf, (uint32_t)ub_size, copy_params,           \
                (rank + 1) % rank_size, EVENT_ID0);                                                              \
            AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                          \
            AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                         \
            aclshmemx_mte_get_nbi(                                                                               \
            ...
            aclshmem_##NAME##_get_nbi(                                                                           \
                dev_gm + dst_offset * 2, gva_gm + src_offset * 2, copy_params, (rank + 1) % rank_size);          \
            AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                          \
            AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                         \
            aclshmem_##NAME##_get_nbi(                                                                           \
                dst_gm[dst_offset * 3], src_gm[src_offset * 3], copy_params, (rank + 1) % rank_size);            \
...
ACLSHMEM_FUNC_TYPE_KERNEL(KERNEL_GET_NUM_NON_CONTIGUOUS);

#define GET_NON_CONTIGUOUS_NUM_TEST(NAME, TYPE)                                 \
    extern "C" __global__ __aicore__ void get_##NAME##_non_contiguous_num_test( \
        GM_ADDR gva, GM_ADDR dev, int repeat, int length, uint64_t config)      \
    {                                                                           \
...
ACLSHMEM_FUNC_TYPE_KERNEL(GET_NON_CONTIGUOUS_NUM_TEST);

#define TEST_NON_CONTIGUOUS_GET(NAME, TYPE)                                                                     \
    void test_##NAME##_non_contiguous_get(                                                                      \
        uint32_t block_dim, void* stream, uint64_t config, uint8_t* gva, uint8_t* dev, int repeat, int length)  \
    {                                                                                                           \
        get_##NAME##_non_contiguous_num_test<<<block_dim, nullptr, stream>>>(gva, dev, repeat, length, config); \
    }

ACLSHMEM_FUNC_TYPE_KERNEL(TEST_NON_CONTIGUOUS_GET);
