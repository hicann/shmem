...
#define put_SIGNAL(NAME, TYPE)                                                                                      \
    ...
    public:                                                                                                         \
        ...
        inline void Process() { aclshmem_##NAME##_put_signal(gva_gm, dev_gm, 16, sig_addr, signal, sig_op, rank); } \
                                                                                                                    \
    private:                                                                                                        \
        TYPE* gva_gm;                                                                                               \
        TYPE* dev_gm;                                                                                               \
...
#define put_SIGNAL(NAME, TYPE)                                                             \
    void putmem_##NAME##_signal_test(                                                      \
        TYPE* gva, TYPE* dev, uint8_t* sig_addr, int32_t signal, int64_t rank, int sig_op) \
    {                                                                                      \
...
#define put_SIGNAL(NAME, TYPE)                                                                                       \
    ...
    {                                                                                                                \
        ...
        int32_t signal_init = 0;                                                                                     \
        ASSERT_EQ(                                                                                                   \
            aclrtMemcpy(signal_addr, sizeof(int32_t), &signal_init, sizeof(int32_t), ACL_MEMCPY_HOST_TO_DEVICE), 0); \
        ...
        ASSERT_EQ(                                                                                                   \
            aclrtMemcpy(                                                                                             \
                output_signal.data(), sizeof(int32_t), signal_addr, sizeof(int32_t), ACL_MEMCPY_DEVICE_TO_HOST),     \
            0);                                                                                                      \
        int32_t flag = 0;                                                                                            \
        for (int i = 0; i < 4; i++) {                                                                                \
            if (output[i] != input[i]) {                                                                             \
...
TEST(TestMemHostApi, TestShmemPutInt32MemSignalAdd)
{
    const int process_count = test_gnpu_num;
    uint64_t local_mem_size = 1024UL * 1024UL * 1024UL;
    test_mutil_task(
        [](int rank_id, int n_rank, uint64_t local_memsize) {
            host_test_aclshmem_int32_mem_signal(rank_id, n_rank, local_memsize, ACLSHMEM_SIGNAL_ADD);
        },
...
#define put_SIGNAL_NBI(NAME, TYPE)                                                         \
    void putmem_signal_##NAME##_test_nbi(                                                  \
        TYPE* gva, TYPE* dev, uint8_t* sig_addr, int32_t signal, int64_t rank, int sig_op) \
    {                                                                                      \
...
#define put_SIGNAL_NBI(NAME, TYPE)                                                                                   \
    ...
    {                                                                                                                \
        ...
        void* ptr = aclshmem_malloc(1024);                                                                           \
        int32_t signal = 6;                                                                                          \
        putmem_signal_##NAME##_test_nbi((TYPE*)ptr, (TYPE*)dev_ptr, (uint8_t*)signal_addr, signal, rank_id, sig_op); \
        ASSERT_EQ(aclrtSynchronizeStream(g_state_host.default_stream), 0);                                           \
        sleep(2);                                                                                                    \
                                                                                                                     \
        ASSERT_EQ(aclrtMemcpy(output.data(), input_size, ptr, input_size, ACL_MEMCPY_DEVICE_TO_HOST), 0);            \
...
TEST(TestMemHostApi, TestShmemPutInt32MemSignalNbiAdd)
{
    const int process_count = test_gnpu_num;
    uint64_t local_mem_size = 1024UL * 1024UL * 1024UL;
    test_mutil_task(
        [](int rank_id, int n_rank, uint64_t local_memsize) {
            host_test_aclshmem_int32_mem_signal_nbi(rank_id, n_rank, local_memsize, ACLSHMEM_SIGNAL_ADD);
        },
        local_mem_size, process_count);
}
