...
#define TEST_PUT_GET(NAME, TYPE)                                                                              \
    ...
    {                                                                                                         \
        ...
        test_##NAME##_put(block_dim, stream, util_get_ffts_config(), (uint8_t *)ptr, (uint8_t *)dev_ptr);     \
        ASSERT_EQ(aclrtSynchronizeStream(stream), 0);                                                         \
                                                                                                              \
        ASSERT_EQ(aclrtMemcpy(input.data(), input_size, ptr, input_size, ACL_MEMCPY_DEVICE_TO_HOST), 0);      \
                                                                                                              \
        test_##NAME##_get(block_dim, stream, util_get_ffts_config(), (uint8_t *)ptr, (uint8_t *)dev_ptr);     \
        ASSERT_EQ(aclrtSynchronizeStream(stream), 0);                                                         \
                                                                                                              \
...
#define TEST_ACLSHMEM_MEM(NAME, TYPE)                                                       \
    void test_##NAME##_aclshmem_mem(int rank_id, int n_ranks, uint64_t local_mem_size)      \
    {                                                                                       \
        ...
        ASSERT_NE(stream, nullptr);                                                         \
                                                                                            \
        test_##NAME##_put_get(stream, (uint8_t *)g_state.heap_base, rank_id, n_ranks);      \
...
#define TESTAPI(NAME, TYPE)                                                         \
    ...
    {                                                                               \
        ...
        test_mutil_task(test_##NAME##_aclshmem_mem, local_mem_size, process_count); \
    }
