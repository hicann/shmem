...
#define TEST_FUNC(NAME, TYPE)                                                                                       \
    extern void multi_instance_##NAME##_test_put(                                                                   \
        uint32_t block_dim, void* stream, uint64_t config, uint8_t* src, uint8_t* dst, uint8_t* shmem_addrs_device, \
        int inst_count)
...
#define TEST_PUT_GET(NAME, TYPE)                                                                                   \
    ...
    {                                                                                                              \
        ...
        ASSERT_EQ(                                                                                                 \
            aclrtMemcpy(                                                                                           \
                shmem_addrs_device, inst_count * sizeof(uint64_t), shmem_addrs_host.data(),                        \
                inst_count * sizeof(uint64_t), ACL_MEMCPY_HOST_TO_DEVICE),                                         \
            0);                                                                                                    \
                                                                                                                   \
        uint32_t block_dim = 1;                                                                                    \
        multi_instance_##NAME##_test_put(                                                                          \
            block_dim, stream, util_get_ffts_config(), (uint8_t*)src, (uint8_t*)dst, (uint8_t*)shmem_addrs_device, \
            inst_count);                                                                                           \
        ...
        for (int loop = 0; loop < inst_count; loop++) {                                                            \
            ASSERT_EQ(                                                                                             \
                aclrtMemcpy(                                                                                       \
                    output_gm.data(), input_size, (char*)dst + loop * input_size, input_size,                      \
                    ACL_MEMCPY_DEVICE_TO_HOST),                                                                    \
                0);                                                                                                \
            ASSERT_EQ(                                                                                             \
                aclrtMemcpy(                                                                                       \
                    output_shmem.data(), input_size, (void*)(shmem_addrs_host[loop]), input_size,                  \
                    ACL_MEMCPY_DEVICE_TO_HOST),                                                                    \
                0);                                                                                                \
                                                                                                                   \
...
#define TESTAPI(NAME, TYPE)                                                                        \
    ...
    {                                                                                              \
        const std::string port_range = get_multi_instance_port_range();                            \
        /* Set before fork so all ranks inherit the same port range. */                            \
        ScopedEnv env_port("SHMEM_INSTANCE_PORT_RANGE", port_range.c_str());                       \
                                                                                                   \
