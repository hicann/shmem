...
void team_all_gather_test(int rank_id, int n_ranks, aclrtStream stream, uint32_t trans_count)
{
    ...
    if (rank_id & 1) {
        ...
        void *dst = (void *)((uint64_t)ptr + aclshmem_team_my_pe(team_odd) * trans_size);
        ASSERT_EQ(aclrtMemcpy(dst, trans_size, input.data(), trans_size, ACL_MEMCPY_HOST_TO_DEVICE), ACL_SUCCESS);

        // Execute AllGather
        team_allgather(1, stream, util_get_ffts_config(), (uint8_t *)ptr, team_odd, trans_count);
...
TEST_F(ShmemTeamAllGatherFuzz, aclshmem_team_all_gather_success)
{
    char fuzzName[] = "aclshmem_team_all_gather_success";
    uint64_t seed = 0;
