...
TEST_F(ShmemHostFuzz, aclshmem_init_success)
{
    char fuzzName[] = "aclshmem_init_success";
    uint64_t seed = 0;
    DT_FUZZ_START(seed, ACLSHMEM_FUZZ_COUNT, fuzzName, 0)
    {
        const int process_count = aclshmem_fuzz_gnpu_num();
...
TEST_F(ShmemHostFuzz, aclshmem_init_custom_attr_success)
{
    char fuzzName[] = "aclshmem_init_custom_attr_success";
    uint64_t seed = 0;
    DT_FUZZ_START(seed, ACLSHMEM_FUZZ_COUNT, fuzzName, 0)
    {
        const int process_count = aclshmem_fuzz_gnpu_num();
        ...
        aclshmem_fuzz_multi_task(
            [](int rank_id, int n_ranks, uint64_t local_mem_size) {
                ...
                aclrtStream stream = nullptr;
                aclshmem_fuzz_test_set_attr(rank_id, n_ranks, local_mem_size, &attributes);
                aclshmem_fuzz_test_init_attr(attributes, &stream);
                ASSERT_EQ(aclshmemx_init_status(), ACLSHMEM_STATUS_IS_INITIALIZED);
                aclshmem_fuzz_test_deinit(stream, aclshmem_fuzz_device_id(rank_id));
            },
            local_mem_size, process_count);
