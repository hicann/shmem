...
TEST_F(ShmemHostHeapFuzz, aclshmem_malloc_free_success)
{
    char fuzzName[] = "aclshmem_malloc_free_success";
    uint64_t seed = 0;
    DT_FUZZ_START(seed, ACLSHMEM_FUZZ_COUNT, fuzzName, 0)
    {
        ...
        aclshmem_fuzz_multi_task(
            [&](int rank_id, int n_ranks, uint64_t local_mem_size) {
                ...
                auto ptr0 = aclshmem_malloc(alloc_size_0);
                auto ptr1 = aclshmem_malloc(alloc_size_1);
...
TEST_F(ShmemHostHeapFuzz, aclshmem_calloc_free_success)
{
    char fuzzName[] = "aclshmem_calloc_free_success";
    uint64_t seed = 0;
    DT_FUZZ_START(seed, ACLSHMEM_FUZZ_COUNT, fuzzName, 0)
    {
        const int process_count = aclshmem_fuzz_gnpu_num();
        ...
        aclshmem_fuzz_multi_task(
            [&](int rank_id, int n_ranks, uint64_t local_mem_size) {
                aclshmem_init_scope scope(rank_id, n_ranks, local_mem_size);
                auto ptr0 = aclshmem_calloc(sizeof(uint64_t), alloc_size_0 / sizeof(uint64_t));
                auto ptr1 = aclshmem_calloc(sizeof(uint64_t), alloc_size_1 / sizeof(uint64_t));
                ASSERT_NE(ptr0, nullptr);
                ASSERT_NE(ptr1, nullptr);
                aclshmem_free(ptr0);
                aclshmem_free(ptr1);
