...
TEST_F(ShmemHostPtrFuzz, aclshmem_get_ptr_success)
{
    char fuzzName[] = "aclshmem_get_ptr_success";
    uint64_t seed = 0;
    DT_FUZZ_START(seed, ACLSHMEM_FUZZ_COUNT, fuzzName, 0)
    {
        ...
        aclshmem_fuzz_multi_task(
            [&](int rank_id, int n_ranks, uint64_t local_mem_size) {
                ...
                int *ptr = (int *)aclshmem_malloc(int_count * sizeof(int));
                ASSERT_NE(ptr, nullptr);

                int *host_self = (int *)aclshmem_ptr(ptr, rank_id);
                ASSERT_NE(host_self, nullptr);
                ASSERT_EQ(host_self, ptr);

                int peer = (rank_id + 1) % n_ranks;
                int *host_remote = (int *)aclshmem_ptr(ptr, peer);
                int *next_remote = (int *)aclshmem_ptr(ptr + 1, peer);
