...
template <class Tp>
static void test_aclshmem_ub_non_contiguous(int rank_id, int n_ranks, uint64_t local_mem_size, device_func put_func,
...
#define TEST_CASE(NAME, TYPE)                                                                                 \
    TEST_F(ShmemUbNonContiguousFuzz, aclshmem_ub_non_contiguous_##NAME##_success)                                \
    {                                                                                                         \
        char fuzzName[] = "aclshmem_ub_non_contiguous_##NAME##_success";                                         \
        uint64_t seed = 0;                                                                                    \
        ...
        {                                                                                                     \
            aclshmem_fuzz_multi_task(                                                                            \
                [](int rank_id, int n_ranks, uint64_t local_mem_size) {                                       \
                    ...
                    test_aclshmem_ub_non_contiguous<TYPE>(rank_id, n_ranks, local_mem_size, put_func, get_func); \
                },                                                                                            \
