...
TEST_F(ShmemSyncP2pFuzz, aclshmem_sync_p2p_success)
{
    char fuzzName[] = "aclshmem_sync_p2p_success";
    uint64_t seed = 0;
    ...
    DT_FUZZ_START(seed, ACLSHMEM_FUZZ_COUNT, fuzzName, 0)
    {
        aclshmem_fuzz_multi_task(
            [](int32_t rank_id, int32_t n_ranks, uint64_t local_mem_size) {
                ...
                int32_t *addr_dev = (int32_t *)aclshmem_malloc(size);
                ASSERT_NE(addr_dev, nullptr);
                ASSERT_EQ(aclrtMemset(addr_dev, size, 0, size), ACL_SUCCESS);
                p2p_chain_do(scope.stream, util_get_ffts_config(), (uint8_t *)addr_dev, rank_id, n_ranks);
