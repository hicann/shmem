...
TEST_F(ShmemSyncOrderFuzz, aclshmem_quiet_order_success)
{
    char fuzzName[] = "aclshmem_quiet_order_success";
    uint64_t seed = 0;
    ...
    DT_FUZZ_START(seed, ACLSHMEM_FUZZ_COUNT, fuzzName, 0)
    {
        aclshmem_fuzz_multi_task(
            [](int32_t rank_id, int32_t n_ranks, uint64_t local_mem_size) {
                ...
                uint64_t *dev_ptr = (uint64_t *)aclshmem_malloc(byte_size);
                ...
                quiet_order_do(scope.stream, util_get_ffts_config(), (uint8_t *)dev_ptr, rank_id, n_ranks);
...
TEST_F(ShmemSyncOrderFuzz, aclshmem_fence_order_success)
{
    char fuzzName[] = "aclshmem_fence_order_success";
    uint64_t seed = 0;
    ...
    DT_FUZZ_START(seed, ACLSHMEM_FUZZ_COUNT, fuzzName, 0)
    {
        aclshmem_fuzz_multi_task(
            [](int32_t rank_id, int32_t n_ranks, uint64_t local_mem_size) {
                ...
                size_t length = 64;
                size_t byte_size = length * sizeof(uint64_t);
                uint64_t *addr_dev = (uint64_t *)aclshmem_malloc(byte_size);
                ...
                fence_order_do(scope.stream, util_get_ffts_config(), (uint8_t *)addr_dev, rank_id, n_ranks);
