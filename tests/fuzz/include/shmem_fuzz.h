...
constexpr int ACLSHMEM_FUZZ_COUNT = 1;                     // 30_000_000;
constexpr int ACLSHMEM_FUZZ_RUNNING_SECONDS = 3 * 60 * 60; // 3小时，单位秒
...
// 使用参数进行默认逻辑的shmem初始化
void aclshmem_fuzz_test_init(int rank_id, int n_ranks, uint64_t local_mem_size, aclrtStream *stream);
...
// aclshmem初始化RAII包装类，适合在 aclshmem_fuzz_multi_task 中使用
class aclshmem_init_scope {
public:
    aclshmem_init_scope(int rank_id, int n_ranks, uint64_t local_mem_size) : rank_id(rank_id)
    {
        std::lock_guard lock(mutex_);
        if (count_ == 0) {
            aclshmem_fuzz_test_init(rank_id, n_ranks, local_mem_size, &stream);
        } else {
            // aclshmem_init_scope should not overlap
            std::cerr << "WARN: aclshmem_init_scope overlapped!" << std::endl;
        }
        count_++;
    }
    ...
    ~aclshmem_init_scope()
    {
        ...
        count_--;
        if (count_ == 0) {
            aclshmem_fuzz_test_deinit(stream, aclshmem_fuzz_device_id(rank_id));
        }
    }

    aclshmem_init_scope(const aclshmem_init_scope &) = delete;
    aclshmem_init_scope(aclshmem_init_scope &&) = delete;
    aclshmem_init_scope &operator=(const aclshmem_init_scope &) = delete;
    aclshmem_init_scope &operator=(aclshmem_init_scope &&) = delete;
