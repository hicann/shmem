...
static shmemx_bootstrap_uid_state_t shmemi_bootstrap_uid_state;
...
static int32_t shmemi_get_uid_magic(shmemx_bootstrap_uid_state_t *innerUId)
{
...
static int32_t shmemi_uid_parse_interface_with_type(const char *ipInfo, char *IP, sa_family_t &sockType, bool &flag)
{
    const char *delim = ":";
...
int32_t shmemi_traverse_ifa(
    struct ifaddrs *ifaddr,
    sa_family_t &sockType,
    bool flag,
    const char **prefixes,
    bool exclude,
    shmemx_bootstrap_uid_state_t *uid_args,
...
int32_t shmemi_get_ip_from_env(shmemx_bootstrap_uid_state_t *uid_args, const char *ipPort) {
    if (uid_args == nullptr || ipPort == nullptr || strlen(ipPort) == 0) {
        SHM_LOG_ERROR("Invalid param: uid_args is null or ipPort is empty");
        return SHMEM_INVALID_PARAM;
    }

    shmemi_get_uid_magic(uid_args);
...
int32_t shmemi_get_ip_from_ifa(shmemx_bootstrap_uid_state_t *uid_args, const char *ipInfo) {
    if (uid_args == nullptr) {
    ...
    if (isIpInfoConfigured) {
        int32_t ret = shmemi_uid_parse_interface_with_type(ipInfo, ifaPrefix, sockType, flag);
    ...
    if (isIpInfoConfigured) {
        ...
        foundValidIp = (shmemi_traverse_ifa(ifaddr, sockType, flag, specifiedPrefixes, false, uid_args, false, allow_local) == SHMEM_SUCCESS);
    } else {
        ...
        foundValidIp = (shmemi_traverse_ifa(ifaddr, sockType, flag, ethPrefixes, false, uid_args, false, allow_local) == SHMEM_SUCCESS);

        if (!foundValidIp) {
            SHM_LOG_WARN("Step 3: Search interfaces exclude 'docker' and 'lo/fe80'");
            foundValidIp = (shmemi_traverse_ifa(ifaddr, sockType, flag, excludePrefixes, true, uid_args, false, allow_local) == SHMEM_SUCCESS);
        }
    }
...
int32_t shmemi_set_ip_info(void *uid, sa_family_t &sockType, char *pta_env_ip, uint16_t pta_env_port,
                          bool is_from_ifa)
{
    // init default uid
    shmemx_bootstrap_uid_state_t *innerUID = (shmemx_bootstrap_uid_state_t *)(uid);
    SHM_LOG_INFO(" ENV IP: " << pta_env_ip << " ENV port: " << pta_env_port << " sockType: " << sockType);
    SHMEM_CHECK_RET(shmemi_get_uid_magic(innerUID));
...
static int shmemi_bootstrap_uid_finalize(shmemi_bootstrap_handle_t *handle) {
    if (!handle) {
        return SHMEM_SUCCESS;
    }
    ...
    if (handle->bootstrap_state) {
        ...
        SHMEM_BOOTSTRAP_PTR_FREE(state->peer_addrs);
        state->peer_addrs = nullptr;
        SHMEM_BOOTSTRAP_PTR_FREE(state);
        handle->bootstrap_state = nullptr;
    }

    if (handle->pre_init_ops) {
        SHMEM_BOOTSTRAP_PTR_FREE(handle->pre_init_ops);
...
static int shmemi_bootstrap_uid_allgather(const void *in, void *out, int len, shmemi_bootstrap_handle_t *handle) {
    ...
    for (int i = 0; i < nranks - 1; i++) {
        ...
        SHMEM_CHECK_RET(socket_send(&state->ring_send_sock, ((char*)out + sslice * len), len), "rank " << rank << ": barrier send failed");
        SHMEM_CHECK_RET(socket_recv(&state->ring_recv_sock, ((char*)out + rslice * len), len), "rank " << rank << ": barrier recv failed");
    }
    return SHMEM_SUCCESS;
}


static int shmemi_bootstrap_uid_barrier(shmemi_bootstrap_handle_t *handle) {
    SHM_LOG_INFO("shmemi_bootstrap_uid_barrier");
    ...
    char token = 0;
    if (rank == 0) {
        SHMEM_CHECK_RET(socket_send(&state->ring_send_sock, &token, 1), "rank 0: barrier send failed");
        SHMEM_CHECK_RET(socket_recv(&state->ring_recv_sock, &token, 1), "rank 0: barrier recv failed");
    } else {
        SHMEM_CHECK_RET(socket_recv(&state->ring_recv_sock, &token, 1), "rank " << rank << ": barrier recv failed");
        SHMEM_CHECK_RET(socket_send(&state->ring_send_sock, &token, 1), "rank " << rank << ": barrier send failed");
    }
    return SHMEM_SUCCESS;
}
...
static int unexpected_enqueue(uid_bootstrap_state* state, int peer, int tag, socket_t* sock) {
    ...
    SHMEM_BOOTSTRAP_CALLOC(&new_conn, 1);
    if (new_conn == NULL) {
        return SHMEM_BOOTSTRAP_ERROR;
    }
...
static int bootstrap_send(void* comm_state, int peer, int tag, void* data, int size) {
    ...
    socket_t sock;
    SHMEM_CHECK_RET(socket_init(&sock, SOCKET_TYPE_BOOTSTRAP, state->magic, &state->peer_addrs[peer]), "bootstrap_send: socket_init failed for peer " << peer);

    SHMEM_CHECK_RET_CLOSE_SOCK(socket_connect(&sock), "bootstrap_send: socket_connect failed for peer " << peer, sock);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_send(&sock, &state->rank, sizeof(int)), "bootstrap_send: send rank failed to peer " << peer, sock);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_send(&sock, &tag, sizeof(int)), "bootstrap_send: send tag " << tag << " failed to peer " << peer, sock);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_send(&sock, data, size), "bootstrap_send: send data (size=" << size << ") failed to peer " << peer, sock);
    if (sock.fd >= 0) {
        socket_close(&sock);
    }
    return SHMEM_SUCCESS;
...
static int bootstrap_recv(void* comm_state, int peer, int tag, void* data, int size) {
    ...
    int found = 0;
    int retry_count = 0;
    int ret = SHMEM_SUCCESS;
    SHMEM_CHECK_RET(unexpected_dequeue(state, peer, tag, &sock, &found));

    if (found == 1) {
        ret = socket_recv(&sock, data, size);
        socket_close(&sock);
        return (ret == SHMEM_SUCCESS) ? SHMEM_SUCCESS : SHMEM_BOOTSTRAP_ERROR;
    }
    while (1) {
        socket_t new_sock;
        int new_peer = -1;
        int new_tag = -1;
        SHMEM_CHECK_RET(socket_init(&new_sock, SOCKET_TYPE_BOOTSTRAP, SOCKET_MAGIC, NULL), "socket_init new_sock failed");
        SHMEM_CHECK_RET_CLOSE_SOCK(socket_accept(&new_sock, &state->listen_sock), "socket_accept new_sock failed", new_sock);
        SHMEM_CHECK_RET_CLOSE_SOCK(socket_recv(&new_sock, &new_peer, sizeof(int)), "socket_recv new_peer failed", new_sock);
        SHMEM_CHECK_RET_CLOSE_SOCK(socket_recv(&new_sock, &new_tag, sizeof(int)), "socket_recv new_tag failed", new_sock);
        if (new_peer == peer && new_tag == tag) {
            SHMEM_CHECK_RET_CLOSE_SOCK(socket_recv(&new_sock, data, size), "socket_recv failed", new_sock);
            return SHMEM_SUCCESS;
        } else {
            SHMEM_CHECK_RET_CLOSE_SOCK(unexpected_enqueue(state, new_peer, new_tag, &new_sock), "unexpected_enqueue failed", new_sock);
        }
    }
}

static int shmemi_bootstrap_uid_barrier_v2(shmemi_bootstrap_handle_t *handle) {
    SHM_LOG_INFO("shmemi_bootstrap_uid_barrier_v2");
    uid_bootstrap_state* state = (uid_bootstrap_state*)(handle->bootstrap_state);
    int rank = state->rank;
    int tag = 0;
    int nranks = state->nranks;
    ...
    for (int mask = 1; mask < nranks; mask <<= 1) {
        ...
        SHMEM_CHECK_RET(bootstrap_send(state, dst, tag, data, sizeof(data)), "rank " << rank << ": barrier send failed, dst: " << dst << "tag: " << tag);
        SHMEM_CHECK_RET(bootstrap_recv(state, src, tag, data, sizeof(data)), "rank " << rank << ": barrier recv failed, src: " << src << "tag: " << tag);
    }
...
static int shmemi_bootstrap_uid_alltoall(const void *sendbuf, void *recvbuf, int length,
                                  shmemi_bootstrap_handle_t *handle) {

}

static void shmemi_bootstrap_uid_global_exit(int status) {
...
static int find_interface_match_subnet(char* ifNames, sockaddr_t* localAddrs, sockaddr_t* remoteAddr) {
    ...
    if (remoteAddr) {
        if (remoteAddr->type == ADDR_IPv4) {
            ...
            SHMEM_CHECK_RET(inet_ntop(AF_INET, &remoteAddr->addr.addr4.sin_addr, ip_str, INET_ADDRSTRLEN) == nullptr, "convert remote ipv4 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
            ...
            SHMEM_CHECK_RET(inet_ntop(AF_INET6, &remoteAddr->addr.addr6.sin6_addr, ip_str, INET6_ADDRSTRLEN) == nullptr, "convert remote ipv6 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
    ...
    SHMEM_CHECK_RET(is_link_local_addr(remoteAddr), "Remote address is link_local", SHMEM_BOOTSTRAP_ERROR);
    ...
    return (found == 0) ? SHMEM_BOOTSTRAP_ERROR : SHMEM_SUCCESS;
}

static int bootstrap_get_sock_addr(socket_t* sock, sockaddr_t* addr) {
    if (sock == NULL) return SHMEM_BOOTSTRAP_ERROR;
...
static int shmemi_bootstrap_net_init(shmemx_bootstrap_uid_state_t* uid_args, bool is_arg_init = true) {
    ...
    if (!is_arg_init) {
        ...
        if (env_ip_port != nullptr) {
            ...
            SHMEM_CHECK_RET(shmemi_get_ip_from_env(uid_args, env_ip_port), 
            ...
            is_from_ifa = true;
            SHMEM_CHECK_RET(shmemi_get_ip_from_ifa(uid_args, env_ifname), 
    ...
    if (priv_info.bootstrap_netinitdone) {
        ...
        if (priv_info.bootstrap_netifaddr.type == ADDR_IPv4) {
            ...
            SHMEM_CHECK_RET(inet_ntop(AF_INET, &priv_info.bootstrap_netifaddr.addr.addr4.sin_addr, ip_str, sizeof(ip_str)) == nullptr, "convert bootstrap_netifaddr ipv4 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
            ...
            SHMEM_CHECK_RET(inet_ntop(AF_INET6, &priv_info.bootstrap_netifaddr.addr.addr6.sin6_addr, ip_str, sizeof(ip_str)) == nullptr, "convert bootstrap_netifaddr ipv6 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
    ...
    if (uid_args->addr.type == ADDR_IPv4) {
        char ip_str[INET_ADDRSTRLEN] = {0};
        SHMEM_CHECK_RET(inet_ntop(AF_INET, &uid_args->addr.addr.addr4.sin_addr, ip_str, sizeof(ip_str)) == nullptr, "convert uid_args addr ipv4 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
        SHM_LOG_INFO(" Root address (IPv4): " << ip_str << ":" << ntohs(uid_args->addr.addr.addr4.sin_port));
    } else if (uid_args->addr.type == ADDR_IPv6) {
        char ip_str[INET6_ADDRSTRLEN] = {0};
        SHMEM_CHECK_RET(inet_ntop(AF_INET6, &uid_args->addr.addr.addr6.sin6_addr, ip_str, sizeof(ip_str)) == nullptr, "convert uid_args addr ipv6 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
...
static int set_files_limit() {
    ...
    SHMEM_CHECK_RET(getrlimit(RLIMIT_NOFILE, &old_limit), "getrlimit failed", SHMEM_BOOTSTRAP_ERROR);
    ...
    SHMEM_CHECK_RET(setrlimit(RLIMIT_NOFILE, &files_limit), "setrlimit failed", SHMEM_BOOTSTRAP_ERROR);
...
static void* bootstrap_root(void* rargs) {
    ...
    SHMEM_BOOTSTRAP_CALLOC(&zero_addr, 1);
    ...
    if (set_files_limit() != 0) {
        ...
        SHMEM_BOOTSTRAP_PTR_FREE(zero_addr);
        SHMEM_BOOTSTRAP_PTR_FREE(args);
        return NULL;
    }
    ...
    while (1) {
        ...
        if (c == 0) {
            ...
            SHMEM_BOOTSTRAP_CALLOC(&rank_addrs, nranks);
            SHMEM_BOOTSTRAP_CALLOC(&rank_addrs_root, nranks);
    ...
    SHMEM_BOOTSTRAP_PTR_FREE(zero_addr);
    SHMEM_BOOTSTRAP_PTR_FREE(rank_addrs);
    SHMEM_BOOTSTRAP_PTR_FREE(rank_addrs_root);
    if (listen_sock != NULL) {
        socket_close(listen_sock);
        SHMEM_BOOTSTRAP_PTR_FREE(listen_sock);
    }
    SHMEM_BOOTSTRAP_PTR_FREE(args);
    return NULL;
}

static int bootstrap_create_root(shmemx_bootstrap_uid_state_t* uid_args) {
    if (uid_args == NULL) {
    ...
    SHMEM_CHECK_RET(SHMEM_BOOTSTRAP_CALLOC(&listen_sock_root, 1), "bootstrap_create_root: malloc listen_sock_root failed");

    // 2. Initialize the listening socket (using the global network interface address)
    SHMEM_CHECK_RET(socket_init(listen_sock_root, SOCKET_TYPE_BOOTSTRAP, uid_args->magic, &uid_args->addr), "bootstrap_create_root: socket_init failed");

    SHMEM_CHECK_RET_CLOSE_SOCK(socket_listen(listen_sock_root), "Listen_sock_root failed while executing listen. fd=" << listen_sock_root->fd, *listen_sock_root);
    ...
    SHMEM_CHECK_RET(SHMEM_BOOTSTRAP_CALLOC(&args, 1), "bootstrap_create_root: malloc args failed");
    ...
    if (ret != 0) {
        ...
        SHMEM_BOOTSTRAP_PTR_FREE(listen_sock_root);
        return SHMEM_BOOTSTRAP_ERROR;
    }
...
int shmemi_bootstrap_get_unique_id(void* uid) {
    ...
    SHMEM_CHECK_RET(shmemi_bootstrap_net_init(uid_args, false), "rank 0: failed to init bootstrap net.");
    SHMEM_CHECK_RET(bootstrap_create_root(uid_args), "rank 0: failed to create root thread");
    return SHMEM_SUCCESS;
}

int shmemi_bootstrap_get_unique_id_static_magic(void* uid, bool is_root) {
    shmemx_bootstrap_uid_state_t* uid_args = (shmemx_bootstrap_uid_state_t*)uid;
...
int shmemi_bootstrap_get_unique_id_by_ipport(void* uid, const char *ipport) {
    shmemx_bootstrap_uid_state_t* uid_args = (shmemx_bootstrap_uid_state_t*)uid;
...
int shmemi_bootstrap_plugin_pre_init(shmemi_bootstrap_handle_t* handle) {
    if (handle->pre_init_ops == nullptr) {
        SHM_LOG_DEBUG(" bootstrap plugin pre init start.");
        SHMEM_CHECK_RET(SHMEM_BOOTSTRAP_CALLOC(&handle->pre_init_ops, 1));
        handle->pre_init_ops->get_unique_id = shmemi_bootstrap_get_unique_id;
        handle->pre_init_ops->get_unique_id_static_magic = shmemi_bootstrap_get_unique_id_static_magic;
...
int shmemi_bootstrap_plugin_init(void* comm, shmemi_bootstrap_handle_t* handle) {
    if (comm == nullptr || handle == nullptr) {
        SHM_LOG_ERROR(" shmemi_bootstrap_plugin_init: invalid arguments (nullptr)");
        return SHMEM_BOOTSTRAP_ERROR;
    }
    socket_t sock, listen_sock_root;
    uid_bootstrap_state* state = nullptr;
    SHMEM_CHECK_RET(SHMEM_BOOTSTRAP_CALLOC(&state, 1));
    shmemx_bootstrap_uid_state_t* uid_args = (shmemx_bootstrap_uid_state_t*)comm;
    sockaddr_t next_addr;
    bootstrap_ext_info info = {};
    int rank = uid_args->rank;
    int nranks = uid_args->nranks;
    
    if (handle->use_attr_ipport && handle->ipport != nullptr) {
        SHM_LOG_DEBUG("shmemi_bootstrap_get_unique_id_by_ipport start. ipport: " << handle->ipport);
        shmemi_bootstrap_get_unique_id_by_ipport(comm, handle->ipport);
    }
    uint64_t magic = uid_args->magic;

    SHMEM_CHECK_RET(shmemi_bootstrap_net_init(uid_args), " rank: " << rank << ": network interface init failed.");
    ...
    SHMEM_CHECK_RET(SHMEM_BOOTSTRAP_CALLOC(&state->peer_addrs, nranks));
    ...
    SHMEM_CHECK_RET(socket_init(&state->listen_sock, SOCKET_TYPE_BOOTSTRAP, state->magic, &priv_info.bootstrap_netifaddr), "State's listen_sock failed while executing init. fd=" << state->listen_sock.fd);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_listen(&state->listen_sock), "State's listen_sock failed while executing listen. fd=" << state->listen_sock.fd, state->listen_sock);
    SHMEM_CHECK_RET(bootstrap_get_sock_addr(&state->listen_sock, &info.ext_addr_listen), "Get addr failed, the listen_sock in state maybe null. fd=" << state->listen_sock.fd);

    SHMEM_CHECK_RET(socket_init(&listen_sock_root, SOCKET_TYPE_BOOTSTRAP, state->magic, &priv_info.bootstrap_netifaddr), "Listen_sock_root failed while executing init. fd=" << listen_sock_root.fd);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_listen(&listen_sock_root), "listen_sock_root failed while executing listen. fd=" << listen_sock_root.fd, listen_sock_root);
    SHMEM_CHECK_RET(bootstrap_get_sock_addr(&listen_sock_root, &info.ext_address_listen_root), "Get addr failed, the listen_sock_root maybe null. fd=" << listen_sock_root.fd);


    SHMEM_CHECK_RET(socket_init(&sock, SOCKET_TYPE_BOOTSTRAP, magic, &uid_args->addr), "Sock failed while executing init. fd=" << sock.fd);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_connect(&sock), "Sock failed while executing connect. fd=" << sock.fd, sock);
    int peer_version = uid_args->version;
    int root_version;
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_send(&sock, &peer_version, sizeof(peer_version)), "Sock failed while executing send peer_version. fd=" << sock.fd, sock);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_recv(&sock, &root_version, sizeof(root_version)), "Sock failed while executing recv root_version. fd=" << sock.fd, sock);
    SHMEM_CHECK_RET(peer_version != root_version, " rank: " << rank << " . version mismatch with root", SHMEM_SMEM_ERROR);
    ...
    if (info.ext_addr_listen.type == ADDR_IPv4) {
        ...
        char ip_str[INET_ADDRSTRLEN] = {0};
        SHMEM_CHECK_RET(inet_ntop(AF_INET, &ipv4->sin_addr, ip_str, sizeof(ip_str)) == nullptr, "convert ext_addr_listen ipv4 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
        ...
        char ip_str[INET6_ADDRSTRLEN] = {0};
        SHMEM_CHECK_RET(inet_ntop(AF_INET6, &ipv6->sin6_addr, ip_str, sizeof(ip_str)) == nullptr, "convert ext_addr_listen ipv6 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
    ...
    if (info.ext_address_listen_root.type == ADDR_IPv4) {
        ...
        char ip_str[INET_ADDRSTRLEN] = {0};
        SHMEM_CHECK_RET(inet_ntop(AF_INET, &ipv4->sin_addr, ip_str, sizeof(ip_str)) == nullptr, "convert ext_address_listen_root ipv4 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
        uint16_t port = ntohs(ipv4->sin_port);
        ...
        char ip_str[INET6_ADDRSTRLEN] = {0};
        SHMEM_CHECK_RET(inet_ntop(AF_INET6, &ipv6->sin6_addr, ip_str, sizeof(ip_str)) == nullptr, "convert ext_address_listen_root ipv6 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
        uint16_t port = ntohs(ipv6->sin6_port);
    ...
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_send(&sock, &info, sizeof(info)), "Sock failed while executing send info. fd=" << sock.fd, sock);
    SHMEM_CHECK_RET(socket_close(&sock), "Sock failed while executing close. fd=" << sock.fd);


    SHMEM_CHECK_RET(socket_init(&sock, SOCKET_TYPE_BOOTSTRAP, SOCKET_MAGIC, nullptr), "Sock failed while executing init. fd=" << sock.fd);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_accept(&sock, &listen_sock_root), "Sock failed while executing accept listen_sock_root. fd=" << sock.fd, sock);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_recv(&sock, &next_addr, sizeof(next_addr)), "Sock failed while executing recv next_addr. fd=" << sock.fd, sock);
    SHMEM_CHECK_RET(socket_close(&sock), "Sock failed while executing close. fd=" << sock.fd);
    SHMEM_CHECK_RET(socket_close(&listen_sock_root), "Listen_sock_root failed while executing close. fd=" << listen_sock_root.fd);

    
    if (next_addr.type == ADDR_IPv4) {
        char ip_str[INET_ADDRSTRLEN] = {0};
        SHMEM_CHECK_RET(inet_ntop(AF_INET, &next_addr.addr.addr4.sin_addr, ip_str, sizeof(ip_str)) == nullptr, "convert next_addr ipv4 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
        ...
        char ip_str[INET6_ADDRSTRLEN] = {0};
        SHMEM_CHECK_RET(inet_ntop(AF_INET6, &next_addr.addr.addr6.sin6_addr, ip_str, sizeof(ip_str)) == nullptr, "convert next_addr ipv6 to string failed. ", SHMEM_BOOTSTRAP_ERROR);
    ...
    SHMEM_CHECK_RET(socket_init(&state->ring_send_sock, SOCKET_TYPE_BOOTSTRAP, magic, &next_addr), "State's ring_send_sock failed while executing init. fd=" << state->ring_send_sock.fd);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_connect(&state->ring_send_sock), "State's ring_send_sock failed while executing connect. fd=" << state->ring_send_sock.fd, state->ring_send_sock);
    SHMEM_CHECK_RET(socket_init(&state->ring_recv_sock, SOCKET_TYPE_BOOTSTRAP, SOCKET_MAGIC, nullptr), "State's ring_recv_sock failed while executing init. fd=" << state->ring_recv_sock.fd);
    SHMEM_CHECK_RET_CLOSE_SOCK(socket_accept(&state->ring_recv_sock, &state->listen_sock),"State's ring_recv_sock failed while executing accept State's listen_sock. fd=" << state->ring_recv_sock.fd, state->ring_recv_sock);
    SHMEM_CHECK_RET(bootstrap_get_sock_addr(&state->listen_sock, state->peer_addrs + handle->mype), "Get addr failed, the listen_sock in state maybe null. fd=" << state->listen_sock.fd);

    SHMEM_CHECK_RET(shmemi_bootstrap_uid_allgather(BOOTSTRAP_IN_PLACE, state->peer_addrs, sizeof(sockaddr_t), handle), "Bootstrap_uid_allgather failed");

    handle->allgather = shmemi_bootstrap_uid_allgather;
    handle->barrier = shmemi_bootstrap_uid_barrier_v2;
