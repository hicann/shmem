...
public:
    ...
    void SetFinished(const shm::acc::AccTcpRequestContext &response) noexcept override
    {
        ...
        if (ret < 0) {
            SHM_LOG_ERROR("unpack response body failed, result: " << ret);
            notify_(IO_ERROR, std::vector<uint8_t>{});
            return;
        }

        if (responseBody.values.empty()) {
            SHM_LOG_ERROR("response body has no value");
            notify_(IO_ERROR, std::vector<uint8_t>{});
            return;
        }

        SHM_LOG_DEBUG("watch end, id: " << response.SeqNo());
...
Result TcpConfigStore::Set(const std::string &key, const std::vector<uint8_t> &value) noexcept
{
    if (key.empty() || key.length() > MAX_KEY_LEN_CLIENT) {
        SHM_LOG_ERROR("key length is invalid");
    ...
    if (response == nullptr) {
        SHM_LOG_ERROR("send set for key: " << key << ", get null response");
        return IO_ERROR;
    }

    auto responseCode = response->Header().result;
    if (responseCode != 0) {
        SHM_LOG_ERROR("send set for key: " << key << ", get response code: " << responseCode);
    }
...
Result TcpConfigStore::GetReal(const std::string &key, std::vector<uint8_t> &value, int64_t timeoutMs) noexcept
{
    ...
    if (responseCode != 0) {
        if (responseCode != NOT_EXIST) {
            SHM_LOG_ERROR("send get for key: " << key << ", resp code: " << responseCode << " timeout:" << timeoutMs);
        }
        return responseCode;
    }
...
Result TcpConfigStore::Add(const std::string &key, int64_t increment, int64_t &value) noexcept
{
    ...
    auto response = SendMessageBlocked(packedRequest);
    if (response == nullptr) {
        SHM_LOG_ERROR("send add for key: " << key << ", get null response");
        return StoreErrorCode::IO_ERROR;
    }

    auto responseCode = response->Header().result;
    if (responseCode != 0) {
        SHM_LOG_ERROR("send add for key: " << key << ", get response code: " << responseCode);
        return responseCode;
    }

    std::string data(reinterpret_cast<char *>(response->DataPtr()), response->DataLen());
    SHM_VALIDATE_RETURN(StrToLong(data, value), "convert string to long failed.", StoreErrorCode::ERROR);
...
Result TcpConfigStore::Remove(const std::string &key, bool printKeyNotExist) noexcept
{
    ...
    auto response = SendMessageBlocked(packedRequest);
    if (response == nullptr) {
        SHM_LOG_ERROR("send remove for key: " << key << ", get null response");
        return StoreErrorCode::IO_ERROR;
    }
    ...
    if (responseCode == StoreErrorCode::NOT_EXIST) {
        if (printKeyNotExist) {
            ...
            SHM_LOG_INFO("send remove for key: " << key << ", is already not exist");
        }
    } else if (responseCode != 0) {
        SHM_LOG_ERROR("send remove for key: " << key << ", get response code: " << responseCode);
    }
...
Result TcpConfigStore::Append(const std::string &key, const std::vector<uint8_t> &value, uint64_t &newSize) noexcept
{
    ...
    auto response = SendMessageBlocked(packedRequest);
    if (response == nullptr) {
        SHM_LOG_ERROR("send append for key: " << key << ", get null response");
        return StoreErrorCode::IO_ERROR;
    }

    auto responseCode = response->Header().result;
    if (responseCode != 0) {
        SHM_LOG_ERROR("send append for key: " << key << ", get response code: " << responseCode);
        return responseCode;
    }
    ...
    long tmpValue = 0;
    SHM_VALIDATE_RETURN(StrToLong(data, tmpValue), "convert string to long failed.", StoreErrorCode::ERROR);
                           ...
                           const std::vector<uint8_t> &value, std::vector<uint8_t> &exists) noexcept
{
    ...
    auto response = SendMessageBlocked(packedRequest);
    if (response == nullptr) {
        SHM_LOG_ERROR("send CAS for key: " << key << ", get null response");
        return StoreErrorCode::IO_ERROR;
    }

    auto responseCode = response->Header().result;
    if (responseCode != 0) {
        SHM_LOG_ERROR("send CAS for key: " << key << ", get response code: " << responseCode);
        return responseCode;
    }
    ...
    uint32_t &wid) noexcept
{
    ...
    SHM_LOG_DEBUG("watch for key: " << key << ", id: " << wid);
    return SM_OK;
}
...
Result TcpConfigStore::Unwatch(uint32_t wid) noexcept
{
    ...
    SHM_LOG_INFO("unwatch for id: " << wid << " success.");
    return SM_OK;
}
...
Result TcpConfigStore::ReceiveResponseHandler(const shm::acc::AccTcpRequestContext &context) noexcept
{
    ...
    if (clientContext == nullptr) {
        SHM_LOG_ERROR("receive response(" << context.SeqNo() << ") not sent request.");
        return SM_ERROR;
    }
