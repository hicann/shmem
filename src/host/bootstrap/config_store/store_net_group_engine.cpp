...
#include "store_monotonic_time.h"
...
SmemGroupEnginePtr SmemNetGroupEngine::Create(const StorePtr &store, const SmemGroupOption &option)
{
    ...
    SHM_ASSERT_RETURN(ss != nullptr, nullptr);

    SmemGroupEnginePtr group = SmMakeRef<SmemNetGroupEngine>(ss, option);
    SHM_ASSERT_RETURN(group != nullptr, nullptr);

    if (option.dynamic) {
        SHM_ASSERT_RETURN(group->StartListenEvent() == SM_OK, nullptr);
    }
    return group.Get();
}
...
Result SmemNetGroupEngine::GroupAllGather(const char *sendBuf, uint32_t sendSize, char *recvBuf, uint32_t recvSize)
{
    SHM_ASSERT_RETURN(store_ != nullptr, SM_INVALID_PARAM);
    uint32_t size = option_.rankSize;
    SHM_ASSERT_RETURN(sendSize * size == recvSize, SM_INVALID_PARAM);
...
bool SmemNetGroupEngine::DealWithListenEvent(std::string& getVal, std::string& prevEvent)
{
    ...
    if (!CharToLong(getVal.c_str() + 1, tmpValue)) {
        SHM_LOG_ERROR("convert string to long failed.");
        return false;
    }
    ...
    SHM_LOG_INFO("handle group event, local_rk:" << option_.rank << " event_rk:" << rk << " event:" << opt);
...
{
    SHM_ASSERT_RETURN(listenSignal_.Initialize() == SM_OK, SM_ERROR);
...
{
    SHM_ASSERT_RETURN(option_.dynamic, SM_INVALID_PARAM);
    if (joined_) {
        return SM_OK;
    }
    ...
    if (TryCasEventKey(val) != SM_OK) {
        SHM_LOG_ERROR("cas event failed, maybe some rank fault!");
        return SM_ERROR;
    }
    ...
    if (option_.joinCb != nullptr) {
        ...
        if (ret != SM_OK) {
            SHM_LOG_ERROR("call join func failed, ret: " << ret);
            goto join_exit;
        }
    }
    ret = store_->Add(SMEM_GROUP_DYNAMIC_SIZE_KEY, 1LL << GROUP_DYNAMIC_SIZE_BIT_LEN | 1, tmp);
    if (ret != SM_OK) {
        SHM_LOG_ERROR("update group dynamic size failed, ret: " << ret);
    }

join_exit:
    auto ret2 = store_->Remove(SMEM_GROUP_LISTEN_EVENT_KEY);
    if (ret2 != SM_OK) {
        SHM_LOG_ERROR("reset group event failed, ret: " << ret2);
    }
...
{
    SHM_ASSERT_RETURN(option_.dynamic, SM_INVALID_PARAM);
    SHM_ASSERT_RETURN(joined_, SM_NOT_STARTED);
    Result ret = 0;
