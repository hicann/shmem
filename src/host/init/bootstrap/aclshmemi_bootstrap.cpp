...
#define BOOTSTRAP_PLUGIN_PREINIT_FUNC "shmemi_bootstrap_plugin_pre_init"
...
int32_t shmemi_bootstrap_pre_init(int flags, shmemi_bootstrap_handle_t *handle) {
    ...
    int (*plugin_pre_init)(shmemi_bootstrap_handle_t *);
...
int32_t shmemi_bootstrap_init(int flags, shmem_init_attr_t *attr) {
    ...
    if (flags & SHMEMX_INIT_WITH_DEFAULT){
        SHM_LOG_INFO("SHMEMX_INIT_WITH_DEFAULT");
    ...
    } else if (flags & SHMEMX_INIT_WITH_MPI) {
        SHM_LOG_INFO("SHMEMX_INIT_WITH_MPI");
        plugin_name = BOOTSTRAP_MODULE_MPI;
        arg = (attr != NULL) ? attr->comm_args : NULL;
    } else if (flags & SHMEMX_INIT_WITH_UNIQUEID) {
        SHM_LOG_INFO("SHMEMX_INIT_WITH_UNIQUEID");
        plugin_name = BOOTSTRAP_MODULE_UID;
        arg = (attr != NULL) ? attr->comm_args : NULL;
    } else {
        SHM_LOG_ERROR("Unknown Type for bootstrap");
