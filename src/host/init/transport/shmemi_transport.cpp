...
int32_t aclshmemi_transport_init(aclshmem_device_host_state_t *g_state, aclshmem_init_optional_attr_t& option_attr) {
    ...
    transport_init_func init_mte_fn = (transport_init_func)dlsym(transport_mte_lib, "aclshmemi_mte_init");
    if (!init_mte_fn) {
    ...
    if (option_attr.data_op_engine_type & ACLSHMEM_DATA_OP_ROCE) {
        ...
        transport_init_func init_rdma_fn = (transport_init_func)dlsym(transport_rdma_lib, "aclshmemi_rdma_init");
        if (!init_rdma_fn) {
