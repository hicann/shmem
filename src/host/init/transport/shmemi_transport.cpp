...
shmemi_host_trans_state_t g_host_state;
...
int32_t shmemi_transport_init(shmemi_device_host_state_t *g_state, shmem_init_optional_attr_t& option_attr) {
    ...
    SHMEM_CHECK_RET(rtGetDeviceInfo(device_id, 0, infoTypeServerId, &server_id));
    const int infoTypeSuperpodId = 29;
    SHMEM_CHECK_RET(rtGetDeviceInfo(device_id, 0, infoTypeSuperpodId, &superpod_id));
    ...
    // If enable RDMA
    if (option_attr.data_op_engine_type & SHMEM_DATA_OP_ROCE) {
...
int32_t shmemi_build_transport_map(shmemi_device_host_state_t *g_state) {
    ...
    for (int j = 0; j < g_host_state.num_choosen_transport; j++) {
        ...
        for (int i = 0; i < g_state->npes; i++) {
            ...
            SHMEM_CHECK_RET(t.can_access_peer(&reach, g_host_state.pe_info + i, g_host_state.pe_info + g_state->mype, &t));
...
int32_t shmemi_transport_setup_connections(shmemi_device_host_state_t *g_state) {
    ...
    for (int i = 0; i < g_state->npes; i++) {
        ...
        if (g_host_state.transport_map[local_offset + i] & 0x1) {
            ...
            shmemi_transport_pe_info_t *my_info = (g_host_state.pe_info + g_state->mype);
