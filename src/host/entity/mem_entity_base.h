...
public:
    ...
    virtual int32_t AllocLocalMemory(
        uint64_t size, hybm_mem_type mType, uint32_t flags, hybm_mem_slice_t& slice) noexcept = 0;
    virtual int32_t RegisterLocalMemory(
        const void* ptr, uint64_t size, uint32_t flags, hybm_mem_slice_t& slice) noexcept = 0;
    ...
    virtual int32_t ExportUserBufferHeap(ExchangeInfoWriter writers[], uint32_t count, uint32_t flags) noexcept = 0;
    virtual int32_t ImportExchangeInfo(
        const ExchangeInfoReader desc[], uint32_t count, void* addresses[], uint32_t flags) noexcept = 0;
