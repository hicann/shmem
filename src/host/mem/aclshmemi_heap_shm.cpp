...
int32_t aclshmemi_symmetric_heap::reserve_heap(size_t size)
{
    ...
    auto peer_guard = aclshmem::make_guard(peer_heap_base_p2p_, [](void **p) {
        if (p != nullptr) {
            std::free(p);
            p = nullptr;
        }
    ...
    if (malloc_physical_memory() != ACLSHMEM_SUCCESS) {
        return ACLSHMEM_SMEM_ERROR;
    }
    auto local_handle_guard = aclshmem::make_guard(local_handle_, [](aclrtDrvMemHandle p) {
        if (p != nullptr) {
    ...
    auto peer_map_guard = aclshmem::make_guard(peer_heap_base_p2p_[rank_id_], [](void *p) {
        if (p != nullptr) {
