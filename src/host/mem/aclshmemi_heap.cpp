...
int aclshmem_symmetric_heap::reserve_heap(size_t size)
{
    ...
    ACLSHMEM_CHECK_RET(aclrtReserveMemAddress(&(peer_heap_base_p2p_[mype]), size, 0, nullptr, 1));
    heap_base_ = peer_heap_base_p2p_[mype];

    // alloc local physical memory
    ACLSHMEM_CHECK_RET(aclrtMallocPhysical(&local_handle, size, &memprop, 0));

    alloc_size = size;
    ACLSHMEM_CHECK_RET(aclrtMapMem(peer_heap_base_p2p_[mype], alloc_size, 0, local_handle, 0));

    return ACLSHMEM_SUCCESS;
}

int aclshmem_symmetric_heap::export_memory()
{
    // Get memory_name
    char memoryName[IPC_NAME_SIZE] = {};
    ACLSHMEM_CHECK_RET(rtIpcSetMemoryName(peer_heap_base_p2p_[mype], alloc_size, memoryName, IPC_NAME_SIZE));

    memory_name = memoryName;
    return ACLSHMEM_SUCCESS;
}

int aclshmem_symmetric_heap::export_pid()
{
    // Get local pid
    ACLSHMEM_CHECK_RET(aclrtDeviceGetBareTgid(&my_pid));

    // Get Sdid
    const int rtModuleTypeSystem = 0;
    const int infoTypeSdid = 26;
    ACLSHMEM_CHECK_RET(rtGetDeviceInfo(device_id, rtModuleTypeSystem, infoTypeSdid, &my_sdid));

    return ACLSHMEM_SUCCESS;
}

int aclshmem_symmetric_heap::import_pid()
{
    // Get all pids
    ...
    for (int i = 0; i < npes; i++) {
        ...
        ACLSHMEM_CHECK_RET(rtSetIpcMemorySuperPodPid(memory_name.c_str(), sdid_list[i], &pid_list[i], 1));
    }

    return ACLSHMEM_SUCCESS;
}

int aclshmem_symmetric_heap::import_memory()
{
    ACLSHMEM_CHECK_RET((g_boot_handle.is_bootstraped != true), "boot_handle not bootstraped, Please check if the method call occurs before initialization or after finalization.", ACLSHMEM_BOOTSTRAP_ERROR);
    ...
    for (int i = 0; i < npes; i++) {
        ...
        ACLSHMEM_CHECK_RET(rtIpcOpenMemory(reinterpret_cast<void **>(&peer_heap_base_p2p_[i]), names[i]));
    }

    return ACLSHMEM_SUCCESS;
}

int aclshmem_symmetric_heap::setup_heap()
{
    ACLSHMEM_CHECK_RET(export_memory());
    ACLSHMEM_CHECK_RET(export_pid());
    ACLSHMEM_CHECK_RET(import_pid());
    ACLSHMEM_CHECK_RET(import_memory());

    return ACLSHMEM_SUCCESS;
}

int aclshmem_symmetric_heap::remove_heap()
{
    for (int i = 0; i < npes; i++) {
        if (i == mype || peer_heap_base_p2p_[i] == NULL) {
            continue;
        }
        ACLSHMEM_CHECK_RET(rtIpcCloseMemory(static_cast<void *>(peer_heap_base_p2p_[i])));
        peer_heap_base_p2p_[i] = NULL;
    }
    ...
    ACLSHMEM_CHECK_RET(rtIpcDestroyMemoryName(memory_name.c_str()));

    ACLSHMEM_CHECK_RET(aclrtUnmapMem(peer_heap_base_p2p_[mype]));
    return ACLSHMEM_SUCCESS;
}

int aclshmem_symmetric_heap::unreserve_heap()
{
    for (int i = 0; i < npes; i++) {
        if (peer_heap_base_p2p_[i] != NULL) {
            ACLSHMEM_CHECK_RET(aclrtReleaseMemAddress(peer_heap_base_p2p_[i]));
        }
    }
    ACLSHMEM_CHECK_RET(aclrtFreePhysical(local_handle));
    return ACLSHMEM_SUCCESS;
}

void *aclshmem_symmetric_heap::get_heap_base()
{
    return heap_base_;
}

void *aclshmem_symmetric_heap::get_peer_heap_base_p2p(int pe_id)
{
