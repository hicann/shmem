...
{
    auto ret = shmem_init_attr(SHMEMX_INIT_WITH_UNIQUEID, &attributes);
    if (ret != 0) {
...
{
    if (bytes.size() < sizeof(shmemx_uniqueid_t)) {
        std::cerr << "Error: Input bytes size (" << bytes.size()
                  << ") is smaller than required size (" << sizeof(shmemx_uniqueid_t)
                  << ")." << std::endl;
        return -1;
    }
    shmemx_uniqueid_t uid;
    ...
    auto ret = shmemx_set_attr_uniqueid_args(rank, world_size, mem_size, &uid, &attr);
    if (ret != 0) {
        std::cerr << "set attr failed " << ret << std::endl;
        return ret;
    }

    return shmem_init_attr(SHMEMX_INIT_WITH_UNIQUEID, attr);
}
...
void DefineShmemAttr(py::module_ &m)
{
    ...
    py::class_<shmem_init_optional_attr_t>(m, "OptionalAttr")
        ...
        .def_readwrite("sockFd", &shmem_init_optional_attr_t::sockFd);
