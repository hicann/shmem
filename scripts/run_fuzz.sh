...
# Run fuzz test
cd "$BUILD_PATH"
"$BUILD_PATH/bin/aclshmem_fuzztest" "$RANK_SIZE" "$IPPORT" "$GNPU_NUM" "$FIRST_RANK" "$FIRST_NPU" --gtest_output=xml:gcover_report/test_detail.xml --gtest_filter=${TEST_FILTER}

# Collect coverage
mkdir -p "$COVERAGE_PATH"
cd "$COVERAGE_PATH"
lcov --d "$BUILD_PATH" --c --output-file "$COVERAGE_PATH/coverage.info" -rc lcov_branch_coverage=1 --ignore-errors mismatch --keep-going --rc lcov_excl_br_line="LCOV_EXCL_BR_LINE|SHM_LOG_*|SHM_ASSERT*|ACLSHMEM_CHECK_RET"
