...
与连续内存区域搬运类接口不同，`_p`/`_g` 仅有线程级形式，不提供 `_block`、`_warp` 变体。因此本样例以 `dim3(1)` 启动 kernel。
...
2. **运行 simt_rma_scalar 示例程序**
   ...
   `run.sh` 默认启动 2 个独立进程，每个进程对应一个 PE 并绑定一张卡，使用 `build/bin/simt_rma_scalar` 执行样例。可通过 `-pes` 参数指定参与运行的 PE（卡）数量，例如启动 4 卡运行：
