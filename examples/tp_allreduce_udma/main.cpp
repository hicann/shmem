...
int ValidateAivCount(int32_t deviceId, uint32_t aivCount)
{
    int64_t vectorCoreNum = 0;
    int status = aclrtGetDeviceInfo(deviceId, ACL_DEV_ATTR_VECTOR_CORE_NUM, &vectorCoreNum);
    if (status != ACL_SUCCESS || vectorCoreNum <= 0) {
        status = aclGetDeviceCapability(deviceId, ACL_DEVICE_INFO_VECTOR_CORE_NUM, &vectorCoreNum);
    }
    if (status != ACL_SUCCESS || vectorCoreNum <= 0) {
        std::cerr << "[ERROR] failed to query Vector Core count, device_id=" << deviceId << ", status=" << status
                  << "\n";
        return status == ACL_SUCCESS ? 1 : status;
    }
    if (aivCount > static_cast<uint64_t>(vectorCoreNum)) {
        std::cerr << "[ERROR] aiv_count exceeds the device Vector Core count, device_id=" << deviceId
                  << ", aiv_count=" << aivCount << ", max=" << vectorCoreNum << "\n";
        return 1;
    }
...
int InitRuntime(const Options& options, int32_t& deviceId, aclrtStream& stream)
{
    ...
    status = ValidateAivCount(deviceId, options.aivCount);
    if (status != ACLSHMEM_SUCCESS) {
        return status;
    }
