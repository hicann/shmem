    ...
    __gm__ void* dst, __gm__ void* src, size_t count, int32_t pe)
{
    if constexpr (OP_TYPE == OpType::None) {
        if constexpr (DATA_SIZE == 8)
            simt::aclshmemx_put8_block(dst, src, 0, pe);
        else if constexpr (DATA_SIZE == 16)
            simt::aclshmemx_put16_block(dst, src, 0, pe);
        else if constexpr (DATA_SIZE == 32)
            simt::aclshmemx_put32_block(dst, src, 0, pe);
        else if constexpr (DATA_SIZE == 64)
            simt::aclshmemx_put64_block(dst, src, 0, pe);
        else
            static_assert(
                DATA_SIZE == 8 || DATA_SIZE == 16 || DATA_SIZE == 32 || DATA_SIZE == 64, "Unsupported DATA_SIZE");
    } else {
        if constexpr (OP_TYPE == OpType::Put) {
            if constexpr (DATA_SIZE == 8)
                simt::aclshmemx_put8_block(dst, src, count, pe);
            else if constexpr (DATA_SIZE == 16)
                simt::aclshmemx_put16_block(dst, src, count, pe);
            else if constexpr (DATA_SIZE == 32)
                simt::aclshmemx_put32_block(dst, src, count, pe);
            else if constexpr (DATA_SIZE == 64)
                simt::aclshmemx_put64_block(dst, src, count, pe);
            else
                static_assert(
                    DATA_SIZE == 8 || DATA_SIZE == 16 || DATA_SIZE == 32 || DATA_SIZE == 64, "Unsupported DATA_SIZE");
        } else { // OpType::Get
            if constexpr (DATA_SIZE == 8)
                simt::aclshmemx_get8_block(dst, src, count, pe);
            else if constexpr (DATA_SIZE == 16)
                simt::aclshmemx_get16_block(dst, src, count, pe);
            else if constexpr (DATA_SIZE == 32)
                simt::aclshmemx_get32_block(dst, src, count, pe);
            else if constexpr (DATA_SIZE == 64)
                simt::aclshmemx_get64_block(dst, src, count, pe);
            else
                static_assert(
                    DATA_SIZE == 8 || DATA_SIZE == 16 || DATA_SIZE == 32 || DATA_SIZE == 64, "Unsupported DATA_SIZE");
        }
...
__aicore__ inline void transfer_kernel(__gm__ void* dst, __gm__ void* src, size_t count, int32_t pe, int32_t ub_size)
{
    if constexpr (VF_TYPE == VfType::Simd) {
        using T = typename TraitTypeFromDataSize<DATA_SIZE>::type;
        if constexpr (OP_TYPE == OpType::None) {
            aclshmemx_mte_put_nbi((__gm__ T*)dst, (__gm__ T*)src, (__ubuf__ T*)(0), ub_size, 0, pe, 0);
        } else if constexpr (OP_TYPE == OpType::Put) {
            aclshmemx_mte_put_nbi((__gm__ T*)dst, (__gm__ T*)src, (__ubuf__ T*)(0), ub_size, count, pe, 0);
        } else /* if OP_TYPE == OpType::Get */ {
            aclshmemx_mte_get_nbi((__gm__ T*)dst, (__gm__ T*)src, (__ubuf__ T*)(0), ub_size, count, pe, 0);
        }
        aclshmemx_mte_quiet();
        ...
        static_assert(VF_TYPE == VfType::Simt || VF_TYPE == VfType::Simd, "Unsupported VF_TYPE");
    }
}
...
// Main test kernel
// -----------------------------------------------------------------------------
__global__ __vector__ void run_demo_mem(__gm__ void* sym_addr, InvokeParameters invp, int32_t ub_size, int32_t frame_id)
{
...
int32_t execute_test_loop(const Config& config, aclrtStream stream)
{
    ...
    for (int32_t block : config.block_sizes) {
        ...
        for (int32_t i = 0; i <= config.bytes_in_exp_max - config.bytes_in_exp_min; i++) {
            ...
            run_demo_mem<<<block, 0, stream>>>(device_mem, invp, config.ub_size * 1024, frame_id);
            ...
            collect_prof_data_to_csv_v2(
                ...
                config.ub_size, config.loop_count, OP_TYPE != OpType::None, csv_data);
        }
    }
    ...
    if (config.mype == ACTIVE_PE) {
        ...
        std::string csv_filename =
            "output/" + std::to_string(DATA_SIZE) + "_" + block_segment + "_" + to_string(OP_TYPE) + "_" +
            to_string(VF_TYPE) + "_" + std::to_string(config.bytes_in_exp_min) + "-" +
            std::to_string(config.bytes_in_exp_max) + "_" + "l" + std::to_string(config.loop_count) + "_" + "t" +
            std::to_string(THREAD_COUNT) + ".csv";
