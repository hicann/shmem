        ...
        if (aivIndex == 0 && aicoreIndex < realAicoreNum) {  // only use one AIV core
            for (uint32_t idx = aicoreIndex; idx < commCoreLoops; idx += realAicoreNum) {
                ...
                for (uint32_t processIndex = 0; processIndex < processLoop; ++processIndex) {
                    ...
                    // [ReduceScatter] 3. Start aclshmem_mte_get_mem_nbi
                    aclshmem_mte_get_mem_nbi(peerMem[outputElemOffset], peerMem[inputElemOffset], buf, copySize,
                ...
                for (uint32_t processIndex = 0; processIndex < processLoop; ++processIndex) {
                    ...
                    while (residueM > 0) {
                        ...
                        if (residueOutputOffset.row() < outputBlockGemmActualSize.m()) {
                            ...
                            // [AllGather] 3. Start aclshmem_mte_get_mem_nbi non-contiguous version
                            aclshmem_mte_get_mem_nbi(params.destination[outputElemOffset], peerMem[inputElemOffset], buf,
