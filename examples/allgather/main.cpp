...
template <class T>
int test_shmem_all_gather(int rank_id, int n_ranks)
{
    // ACLStream init
    int status = 0;
...
int main(int argc, char *argv[])
{
    ...
    if (std::string(data_type) == "int") {
        status = test_shmem_all_gather<int>(rank_id, n_ranks);
    } else if (std::string(data_type) == "int32_t") {
        status = test_shmem_all_gather<int32_t>(rank_id, n_ranks);
    } else if (std::string(data_type) == "float16_t") {
        status = test_shmem_all_gather<fp16_t>(rank_id, n_ranks);
    } else if (std::string(data_type) == "bfloat16_t") {
        status = test_shmem_all_gather<bfloat16>(rank_id, n_ranks);
    }
