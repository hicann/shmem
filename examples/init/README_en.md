...
**To run workflows other than default, you must manually install MPI and import the corresponding environment variables.** [Installation Reference](https://www.hiascend.com/document/detail/en/canncommercial/850/devaids/hccltool/HCCLpertest_16_0002.html)
...
Run the build script in the root (`shmem/`) directory:
...
Execute the uid_multi_stress workflow with 4 PEs (loop stress test of uid_multi):

```bash
# STRESS_LOOP_COUNT=20 is defined at build time to run the uid_multi create/destroy workflow 20 times in a loop.
# Replace enpxxx with the NIC obtained by running the ip addr command.
export SHMEM_UID_SOCK_IFNAME=enpxxxxxxx:inet4
...
To change the number of loops, edit the `STRESS_LOOP_COUNT` value of RUN_MODE=6 in CMakeLists.txt and run the case again.
...
> **Note**: For `-ipport`, pass in only `host:port` (the script automatically adds the `tcp://` prefix).
>
> - IPv4 address: `192.168.1.100:8666`
> - **Host name**: `my-server:8666` (the host name is resolved to the actual IP address by the system DNS)

Execute the default workflow on two servers, with 2 PEs on each server. Run the following commands on both servers:

```bash
# Server where PE0 is located (${IP address:port number of this server} can also be a host name, such as my-server:8666)
bash run.sh -mode default -pesize 4 -fpe 0 -gnpus 2 -ipport ${IP address:port number of this server}
# Other servers
...
**mpirun requires that the locations of the executable files on multiple servers be the same. Compile the samples on all servers in the same mode first. If the modes are inconsistent, an error will be reported or the system will hang.**

```sh
bash run.sh -build -mode mpi # For the UID workflow, change the mode parameter value to uid.
```

Then, run the following commands on the server where PE0 is located:
MPI workflow:
...
**If you want to execute a single-server test case again in MPI or UID mode, you are advised to delete the hostfile file.**
